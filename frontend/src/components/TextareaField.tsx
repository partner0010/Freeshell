...
export default function TextareaField({
  ...
  const fieldId = id ?? (baseId ? `textarea-${baseId}` : "textarea-field");
  ...
  const statusIconByState: Record<Exclude<NonNullable<TextareaFieldProps["status"]>, "default">, string> = {
    success: "✓",
    warning: "!",
    error: "×",
  };
      ...
      <div className="relative mt-2">
        ...
        {prefixSlot ? (
          prefixClickable ? (
            <button
              type="button"
              className={`absolute left-2 top-2 rounded px-1 text-sm ${
                prefixDisabled
        ...
        {actionSlot ? (
          ...
          suffixClickable ? (
            <button
              type="button"
              className={`absolute right-2 top-2 rounded px-1 text-sm ${
                suffixDisabled
                  ? "text-neutral-300 opacity-60"
            ...
            <span
              className={`pointer-events-none absolute right-3 top-3 text-sm ${
          ...
          <span className="pointer-events-none absolute right-3 top-3 text-sm text-neutral-400">
            {resolvedStatusIcon}
          </span>
        ) : null}
      </div>
